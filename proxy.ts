...
export async function middleware(req: NextRequest) {
  ...
  // =========================================================
  // 🔥 AUTHENTICATED USER CHECK (BLOCKED / VERIFIED)
  // =========================================================
  if (token?.email && token?.role) {
    const config = roleConfig[token.role as keyof typeof roleConfig];
    if (!config) return NextResponse.next();
  ...
  // =========================================================
  // 🔁 LOGGED-IN USERS ACCESSING LOGIN PAGE (FIXED LOOP)
  // =========================================================
  if (token && isAuthPage) {
    const config = roleConfig[token.role as keyof typeof roleConfig];
    ...
    // 🔥 HANDLE CANDIDATE STREAM (UNCHANGED)
    if (token.role === "CANDIDATE") {
      if ((token as any).stream) {
        destination = "/home/enrolled";
      } else {
  ...
  // =========================================================
  // ❌ NOT LOGGED IN (FIXED LOOP)
  // =========================================================
  if (!token) {

    // 🔥 CRITICAL FIX → allow home temporarily (prevents loop)
